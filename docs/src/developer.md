...
3. Add this repo as a remote:
   ...
   ```bash
   git remote add upstream https://github.com/langestefan/CableLineModels.jl
   ```
...
We use [prek](https://github.com/j178/prek) to run the linters and formatters. It reads the same `.pre-commit-config.yaml` as [pre-commit](https://pre-commit.com).
The Julia code is formatted with [Runic.jl](https://github.com/fredrikekre/Runic.jl). The Runic hook installs its own copy of Runic, so the only requirement is a `julia` on your `PATH`.

Install prek following the instructions in its [README](https://github.com/j178/prek), then activate it as a git hook:
...
**Now, you can only commit if all the hooks pass**.
...
- Go back to main screen and click on the latest commit (link: <https://github.com/langestefan/CableLineModels.jl/commit/main>)
...
- After the merge happens, TagBot will trigger and create a new GitHub tag. Check on <https://github.com/langestefan/CableLineModels.jl/releases>
- After the release is create, a "docs" GitHub action will start for the tag.
- After it passes, a deploy action will run.
- After that runs, the [stable docs](https://langestefan.github.io/CableLineModels.jl/stable) should be updated. Check them and look for the version number.
